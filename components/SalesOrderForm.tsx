...
import type { SalesOrder, SalesOrderItem, Product, Customer, User } from '../types';
...
export const SalesOrderForm: React.FC<SalesOrderFormProps> = ({ onSave, onClose, orderToEdit, currentUser }) => {
  ...
  useEffect(() => {
    if (orderToEdit) {
      ...
      if (isAdmin && orderToEdit.subtotal > 0) {
        const percentage = (orderToEdit.discount / orderToEdit.subtotal) * 100;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const orderData: SalesOrder = {
      ...
      orderNumber: orderToEdit?.orderNumber || `SO-${Date.now()}`,
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex justify-end gap-4 pt-4 border-t border-gray-700">
        ...
        <button type="submit" className="px-5 py-2 bg-sky-600 text-white rounded-lg hover:bg-sky-700">حفظ الطلب</button>
      </div>
    </form>
  );
};